...
### Honest state of the system (v0.14.0)

| Area | State |
|---|---|
| Deterministic transform primitives (v0.14.0) | 🟩 A closed, Ajv-validated registry (`src/core/transforms/`) the LLM only selects/parameterizes, never freehands — Tier 1 (code, 3 primitives) + Tier 2 (declarative YAML, a constrained no-code-execution template engine). Wired as a new tier ahead of the existing freehand-LLM/template fallback in 3 of 5 codegen agents (not `dataModelerAgent`'s multi-table schema design, nor the marts model). See §17.13. |
| Declarative Specification Framework — Pipeline Spec (v0.14.0) | 🟩 A strictly-validated (`additionalProperties:false` at every level), deterministically-compiled YAML spec + a Markdown design doc generated alongside it, both derived from an approved BPS plus its full surrounding context. New opt-in "Generate Pipeline Spec" command, additive to the existing BPS→plan flow, not a replacement. See §5.7, §17.15. |
| Primitive lifecycle & governance (v0.14.0) | 🟩 Sidebar Primitives palette (list/preview/publish/deprecate); `draft→published→deprecated→retired`; an opt-in permission gate (`primitiveManagementEnabled`, default off); version pinning so an approved Pipeline Spec survives a primitive being republished later. See §17.14. |
| Platform Onboarding & Credential Architecture | 🟨 **Designed (2026-09-17), not yet implemented.** A `CredentialResolver`/`CredentialReference` abstraction distinguishing interactive-user vs. service-account principals, and a CLI-detection cascade for `snow`/`databricks` mirroring `claudeCodeAdapter.ts`'s existing pattern. See `requirements.md` §17.6/§16.8 — this is the next thing to pick up, ahead of wiring the adapters below. |
...
| Tool-executing skills (`toolSkillAgent`) | 🟨 Import a Claude Agent Skill (`SKILL.md`) and run it with real tool access — implemented narrower than originally scoped (v0.9.0, Phase D). `claude`: real, verified end-to-end for Read/Grep/Glob/Edit/Write (Bash unverified); one whole-run confirmation, not per-call. `copilot`: a real `vscode.lm` tool-calling loop with true per-call approval + audit — logic correct, **not exercised against a live Copilot session**. Not native Claude Code plugin loading; not a hard OS sandbox. Not reachable from the auto-planner (explicit invocation only). **Confirmed still open (v0.14.0 review):** `declaredTools` is parsed but never enforced anywhere, and only 2 of 7 LLM providers can run a skill at all — flagged as a hard prerequisite for the also-planned Superpowers/Git-sourced skill import, recommended to be pulled forward alongside the platform-onboarding work above. See requirements.md §9a, §16.8. |
...
| Data platform adapters (Snowflake / Databricks) | 🟥 **Stubs, unchanged since v0.8.0.** `connect()` validates params but opens no connection; `executeQuery()` returns `[]`. End-to-end metadata extraction does not work. Now explicitly sequenced as **Phase 3b**, gated on Phase 3a's credential architecture (designed, not yet implemented) landing first — see the row above and `requirements.md` §16.1/§16.8. |
| `snowflake-sdk` real execution | 🟥 Only `src/spokes/snowflakeExecutor.ts` imports it, with a **hardcoded username** bug (unchanged — still open, see §16 R2). Phase 3a's design would source this via a resolved `CredentialReference` instead. |
| Vector engine / worker threads / `js-tiktoken` | 🟥 Not implemented |
| `deactivate()` cleanup | 🟩 **Fixed, v0.14.0.** `src/core/disposables.ts#DisposableRegistry` — `activate()` registers a disposal callback for the reassignable `connectionManager`; `deactivate()` calls `disposeAll()`. Previously R6/empty, see §16 (now resolved). |
| Tests | 🟩 128 functional tests, all in one file (`test/functional.test.cjs`) — up from 90 at v0.13.0. New coverage (v0.14.0): transform primitives (Tier 1 + Tier 2), the primitive registry/lifecycle/version-pinning, the Pipeline Spec schema/compiler/manager, attachment extraction, and `DisposableRegistry`. |
...
**Bottom line for you:** the *spec → business-problem-confirm → phases → context → context-approve → plan → plan-approve → stages-confirm → artifact* pipeline is real, gated end-to-end, and demoable with an LLM (Copilot or Claude works with zero API key). As of v0.14.0, a parallel, stricter path exists too: *approved spec → Pipeline Spec (strict, deterministically compiled) → design doc* — see §5.7. The *live database connectivity* is still not real — that hasn't changed since v0.8.0, but it's no longer just "someday": a credential architecture (CLI reuse + service-account resolvers) was designed 2026-09-17 specifically to unblock it, sequenced as Phase 3a before the adapters themselves (Phase 3b). Most v0.9.0–v0.13.0 work was about making the *governed lifecycle* (spec → context → plan → artifacts) consistent and impossible to accidentally bypass; v0.14.0's work is about making step *content* deterministic wherever a vetted primitive fits, and giving AutoDE a genuinely machine-compilable spec artifact it never had before.
...
| **Runtime deps** | `ajv` (schema validation — also now backs the transform-primitive and Pipeline Spec schemas), `yaml` (parsing), `snowflake-sdk` (declared, barely used) |
| **Dev deps** | `@types/node`, `@types/vscode`, `typescript`, plus (v0.14.0) `eslint`/`@typescript-eslint/*` — a first-pass config, conservative (warnings, not errors) on rules likely to flag pre-existing code. |
| **Total tracked files** | Grown materially since v0.13.0 with `src/core/transforms/`, `src/core/pipelineSpec/`, and their tests — see §3 for the new directories; run `git ls-files \| wc -l` for a current count rather than trusting this number as it ages. |
...
11. (v0.14.0) Generate a **Pipeline Spec** from the approved BPS — a strictly-validated YAML compiled deterministically (no LLM freehand) into artifacts, with a Markdown design doc rendered alongside it — via "AutoDE: Generate Pipeline Spec," additive to the plan flow above.
12. (v0.14.0) Manage the **transform-primitive catalog** from the sidebar Primitives section: preview a primitive's compiled output, and (if `primitiveManagementEnabled`) publish/deprecate a declarative (Tier 2) one authored as plain YAML — no TypeScript, no rebuild.
...
| Deterministic execution / declarative spec compilation (v0.14.0) | 🟩 | `src/core/transforms/`, `src/core/pipelineSpec/` |
...
| `types.ts` | **The type dictionary.** `BusinessProblemSpec`, `PlanState`, `PlanStep`, `AgentType`, `TargetEnvironment`, `GeneratedArtifact`, `IntakeSession`, `SpecEngineAction`, `DataAgentHubSettings`, `AttachmentExtract`, `PrimitiveLifecycleStatus` (v0.14.0), … | everywhere | — |
| `disposables.ts` (v0.14.0) | **Pure.** `createDisposableRegistry()` — the fix for `deactivate()` (§16, R6, now resolved): `activate()` registers a callback closing over its local `connectionManager`; `deactivate()` calls `disposeAll()`. Exists because `deactivate()` has no access to `activate()`'s closure otherwise. | `extension.ts` | nothing |
| `attachmentExtraction.ts` (v0.14.0) | **Pure.** `buildAttachmentExtractionPrompt()`/`parseAttachmentExtract()`/`fallbackAttachmentExtract()` — structured-fact extraction from a spec-discovery attachment, mirroring the `specOpsPrompts.ts`/`specSynthesis.ts` split. Never throws; degrades to a plain summary. | `agentHub.ts` (`extractAttachmentFacts`) | `types` |
| `transforms/` (v0.14.0) | **The deterministic execution core.** `types.ts` (`TransformPrimitive` contract), `registry.ts` (Tier 1 + Tier 2 merged catalog, version-pinned resolution), `primitives/*.ts` (Tier 1: `renameCast`, `dedup`, `incrementalLoad`), `declarative/*.ts` (Tier 2: YAML `PrimitiveDefinition`, a constrained no-code-execution template engine, the bundled+override loader, `lifecycle.ts` publish/deprecate). See `technical-design.md` §12.2, §17.13. | `llmParamSelector.ts`, 3 of 5 codegen agents, `pipelineSpec/compiler.ts`, `webviewProvider.ts` (Primitives palette) | `ajv`, `../../context/Yaml` |
| `pipelineSpec/` (v0.14.0) | **The Pipeline Spec framework.** `types.ts`/`schema.ts` (strict, `additionalProperties:false` at every level), `validator.ts` (Ajv), `compiler.ts` (zero-LLM deterministic compilation via `transforms/registry.ts`), `designDocGenerator.ts` (pure Markdown + Mermaid rendering). See `technical-design.md` §12.3. | `context/PipelineSpecManager.ts`, `agents/pipelineSpecSynthesis.ts` | `transforms/` |
...
| `PipelineSpecManager.ts` (v0.14.0) | **Persists the Pipeline Spec.** Modeled function-for-function on `SpecManager.ts`: `<contextRoot>/spec/pipeline.yaml` + `spec/history/`, atomic writes, `approve()` doesn't bump version, a revision from an approved predecessor does. A `specVersion` mismatch loads leniently (a warning) rather than throwing. | The AutoDE analogue of a compile-ready contract — see §5.7. |
| `AttachmentStore.ts` (v0.14.0) | Persists a spec-discovery attachment (raw content + `AttachmentExtract`) to `<contextRoot>/attachments/<id>.yaml`, atomic write — survives past the ephemeral `IntakeSession` that created it, so Pipeline Spec synthesis can read it later. | `list()` tolerates a corrupt sibling file without failing the whole load. |
...
**v0.14.0 note:** wiring these two adapters for real (Phase 3b) is now explicitly sequenced *after* a credential-resolution foundation (Phase 3a, designed 2026-09-17, not yet implemented) — a `CredentialResolver`/`CredentialReference` abstraction distinguishing interactive-user (CLI reuse, mirroring `claudeCodeAdapter.ts`'s detection cascade) from service-account (env vars, a pluggable secrets-manager extension point) principals. See `requirements.md` §17.6/§16.8, `technical-design.md` §4.8. `providerRegistry.ts` above gets consumed for the first time as part of this.
...
### `src/spokes/` — one real file, one no-longer-relevant note

- `src/spokes/snowflakeExecutor.ts` — **still wired** (imported by `agentHub.ts`), the only file with a real `snowflake-sdk` connection.
- **Removed in v0.14.0 (Phase 1 stabilization):** `src/spokes/{architectureAgent,ingestionAgent,sttmAgent}.ts` and the entirely-dead `src/features/**` (a near-verbatim copy of the above under a deeper path, zero imports anywhere) were confirmed dead and deleted, along with two stale `.vsix` build artifacts from an earlier product name. If you're reading an older mirror of this doc that still describes them, that content is now historical.
...
### 5.7 Workflow: Approved Spec → Pipeline Spec (v0.14.0, additive to §5.3)

The strict, deterministically-compiled counterpart to the plan flow in §5.3 — same starting point (an approved BPS with Target Context approved, Source Context approved-or-not-applicable), different destination (a Pipeline Spec, not a freehand LLM plan).

```
"AutoDE: Generate Pipeline Spec" command
 → webviewProvider.generatePipelineSpec()
     computeContextGateStatus(spec)   ← the exact same gate §5.3's Generate Plan uses
     if not ready: post('error', {message: '...'})   ← no separate gate to maintain
     attachmentStore.list()   → AttachmentExtract[] (v0.14.0, §17.4 of requirements.md)
 → hub.synthesizePipelineSpec({bps, targetContext, sourceContext, contextLayerText, attachmentExtracts, previous})
     up to 3 attempts: buildPipelineSpecSynthesisPrompt() → LLM → parsePipelineSpecResponse()
     an Ajv validation failure feeds its errors into the next attempt's prompt, not a silent retry
 → pipelineSpecManager.saveSpec(draft)   ← same atomic-write/history pattern as SpecManager
 → generateDesignDoc(draft) written to spec/design.md   ← pure function, no LLM, always matches the YAML
 → post('pipelineSpecGenerated', {spec, designDoc})
```

Approving it (`approvePipelineSpec` → `pipelineSpecManager.approve()`) is where the *other* new flow becomes available: `compilePipelineSpec(spec, dialect)` (`core/pipelineSpec/compiler.ts`) walks every entity's `transforms[]` through the existing `compileTransformSpec()` (§17.13) with **zero LLM calls and no freehand fallback** — a compile failure here means the approved spec itself is wrong, not a coverage gap. Nothing today automatically triggers this compile step end-to-end into real artifacts; that wiring is part of Phase 3b (real adapter execution), not yet built.
...
|---|---|---|---|
| R1 | **Adapters are stubs.** `connect()` opens nothing; `executeQuery()` returns `[]`. | Source assessment, metadata extraction, and any "real data" story are non-functional. The whole Discover phase is theater. | **Now Phase 3b, gated on Phase 3a landing first** (`requirements.md` §16.1/§16.8) — a credential-resolution foundation (CLI reuse + service-account resolvers), designed 2026-09-17, not yet implemented, so the adapters authenticate through it from day one rather than a flat settings assembly needing a second migration. |
...
| R6 | ✅ **Resolved (v0.14.0).** ~~`deactivate()` is empty.~~ `src/core/disposables.ts#DisposableRegistry` — `activate()` registers a callback closing over its local `connectionManager` (the one resource `deactivate()` otherwise has no access to, since it lives in `activate()`'s closure); `deactivate()` calls `disposeAll()`. | — | Not yet extended to `ContextFileManager`/`TargetConfigManager`/the graph/other watchers — only `connectionManager` is registered today. A reasonable next increment if more resources need disposal, using the same registry. |
| R7 | ✅ **Resolved (v0.14.0).** ~~Dead/duplicate code: `src/features/**`, the 3 superseded `src/spokes/*` files.~~ Confirmed zero imports (`grep -rn "features/" src/`), then deleted, along with two stale `.vsix` build artifacts from an earlier product name. | — | `snowflakeExecutor.ts` was **not** moved into `src/agents/`/`src/dqm/` in this pass — it's still the sole real `snowflake-sdk` call site in `src/spokes/`, a reasonable candidate to revisit during Phase 3b. |
...
| R12 | ✅ **Resolved (v0.14.0).** ~~No CI, no lint, tests not in `npm test`.~~ `.github/workflows/ci.yml` (compile→test→lint) + a first-pass ESLint config (conservative — warnings, not errors, on rules likely to flag pre-existing code) + `npm test` wired to the existing suite. | — | The ESLint ruleset is deliberately not strict yet (0 errors, ~35+ pre-existing warnings tracked but not blocking) — tighten incrementally rather than in one pass. |
...
| R24 | **Only 3 of the 5 codegen agents (v0.14.0) have a primitive-eligible path** (`ingestionAgent`, `sttmAgent`, `transformScaffoldAgent`'s staging model). `dataModelerAgent` (multi-table schema design) and the marts model remain freehand-LLM-only — a deliberate scope decision, not an oversight, since a single-source transform primitive is a poor fit for either. Only 3 Tier-1 primitives exist (`rename_cast`/`dedup`/`incremental_load`), so for most real tasks the LLM still won't find a match and falls through to freehand generation anyway — the deterministic path is currently the exception, not the rule. | The "LLM never freehands execution logic" property only holds for a narrow slice of real work today. | Grow Tier-2 (declarative) primitive coverage — that's the whole point of the extensibility framework (§17.13/§17.14): no code change needed, just author more `PrimitiveDefinition` YAML files. |
| R25 | **Superpowers/vendor-skill integration is designed but blocked on tool-skill enforcement work.** The v0.14.0 platform-onboarding review flagged that extending `AutoDE: Import Tool Skill` with a Git-URL source is "nearly worthless" while tool-skill execution only works on 2 of 7 LLM providers and `declaredTools` is parsed but never enforced (see the Tool-executing skills row in §1, R17/R18/R19 above). | Importing a Superpowers skill today would only be genuinely usable on Claude/Copilot, undermining "regardless of which LLM a user selects." | Generalize `ToolSkillAgent.ts`'s execution loop to any provider exposing `completeWithTools()` and wire real `allowed-tools` filtering, *before* or alongside building the Git-import source — see `requirements.md` §16.8. |
| R26 | **`compilePipelineSpec()` (v0.14.0) has no automatic caller.** Approving a Pipeline Spec persists it, but nothing currently walks `compilePipelineSpec()`'s output into real artifacts/execution — that wiring depends on Phase 3b's real adapters, not yet built. | The Pipeline Spec flow is fully real up through "approved, compilable," but "compiled SQL actually runs against a warehouse" isn't connected yet. | Wire this once Phase 3a/3b land — `compilePipelineSpec()` itself doesn't need to change, it just needs a caller. |
...
### 17.13 Add a new Tier-2 (declarative) transform primitive — no TypeScript, no rebuild (v0.14.0)

This is the whole point of the extensibility framework in `technical-design.md` §12.2 — a new primitive is one YAML file:

1. Create `.ai-context/primitives/<kind>.yaml` (or a bundled one under `primitive-definitions/` if it should ship with the extension): `kind`, `version: 1`, `status: draft`, `description`, `paramSchema` (an Ajv JSON Schema — copy the shape from an existing Tier-1 primitive's `paramSchema`, e.g. `src/core/transforms/primitives/dedup.ts`), `platformTemplates: { default: "..." }` (a `{{param}}`/`{{join arr ", "}}`/`{{#each}}...{{/each}}` template — see `src/core/transforms/declarative/templateEngine.ts` for the exact supported syntax; there is **no** conditional/expression support by design), `previewParams` (sample values matching `paramSchema`, used by the sidebar's Preview button).
2. Open the sidebar's Workflow Palette → Primitives section → your new `kind` should already appear (tagged "Tier 2 · draft") after the next `ensurePrimitivesLoaded()` reload (reopen the palette, or restart the Extension Development Host). Click **Preview** to confirm it renders against `previewParams` before anything else touches it.
3. If it should be selectable by the LLM, it needs `primitiveManagementEnabled: true` (setting, default off) and an explicit **Publish** — a `draft` primitive is loaded/previewable but never offered to `selectTransformSpec()`'s candidate catalog (§12.5).
4. Nothing in `src/core/transforms/registry.ts` needs to change — `TRANSFORM_PRIMITIVES` is a live merge, and `compileTransformSpec()`/`selectTransformSpec()` don't know or care which tier a `kind` came from.
5. If a codegen agent should actually *offer* this new kind to the LLM, add it to that agent's `candidateKinds` array (see `IngestionPipelineAgent.ts`'s call to `generateViaPrimitiveOrFallback()` for the pattern) — a primitive existing in the registry doesn't automatically mean any agent will try it.

**When to reach for Tier 1 (TypeScript) instead:** only when the logic genuinely can't be expressed as a bounded template — e.g. `dedup.ts`'s dialect-specific `EXCLUDE`/`EXCLUDE` branching. That's the signal to write a real `TransformPrimitive` implementation and add it to `registry.ts`'s `TRANSFORM_PRIMITIVES`, with a code review — not a reason to grow the template engine.

### 17.14 Publish/deprecate a primitive definition, and what version pinning means for you

- **Publish** (`declarative/lifecycle.ts#publishPrimitiveDefinition`): a *first* publish (current status isn't already `published`) keeps `version` as-is. **Republishing an already-published, edited definition bumps `version` and archives the prior revision** to `.ai-context/primitives/history/<kind>.v<version>.yaml` — same convention as `SpecManager`/`PipelineSpecManager`. Both actions require `primitiveManagementEnabled: true`; the sidebar hides the buttons entirely, not just disables them, when it's off.
- **Deprecate**: flips `status` to `deprecated` — excluded from new LLM selections (`selectTransformSpec()`) but **still compiles** for anything that already references it (`compileTransformSpec()` doesn't check status at all).
- **If you're building something that generates a Pipeline Spec entity referencing a primitive**, and you need its compiled meaning to never change even if someone republishes a newer version later, set `TransformSpec.primitiveVersion` to the version you're pinning to. `registry.ts`'s `resolvePrimitive()` looks that exact version up in a separate `VERSIONED_PRIMITIVES` map (populated from `history/` by `webviewProvider.ts#ensurePrimitivesLoaded()`) before falling back to whatever's currently live.

### 17.15 Generate a Pipeline Spec for the current business problem (v0.14.0)

1. Approve a BPS, then build+approve Target Context (and Source Context, or confirm Not Applicable for Greenfield) — the exact same gate (`computeContextGateStatus()`) Generate Plan already requires; there's no separate readiness bar to clear.
2. Run "AutoDE: Generate Pipeline Spec" (Command Palette). This calls `AgentHub.synthesizePipelineSpec()` (§12.3 of `technical-design.md`), which retries up to 3 times against Ajv validation errors before giving up.
3. Review the result: `.ai-context/problems/<id>/spec/pipeline.yaml` (the strict spec) and `spec/design.md` (its deterministic Markdown projection — regenerate it any time via `generateDesignDoc(spec)`, it's a pure function).
4. Approve it (`AgentHub` → `pipelineSpecManager.approve()`, same as approving a BPS). Once approved, `compilePipelineSpec(spec, dialect)` (`core/pipelineSpec/compiler.ts`) will deterministically compile every entity's `transforms[]` — but nothing calls this automatically yet (R26, §16); if you need compiled output today, call it directly.
5. If you need a primitive that doesn't exist yet while authoring, see §17.13 — a Tier-2 primitive is available to synthesis the moment it's loaded (draft is fine for testing; publish it before relying on the LLM to select it in a real run).
...
| **Transform primitive / `TransformPrimitive`** | (v0.14.0) A closed-shape, Ajv-validated `{kind, paramSchema, compile()}` contract (`src/core/transforms/types.ts`) an LLM selects and parameterizes but never writes code for — the "spec compiles to code" boundary. Tier 1 = code-reviewed TypeScript (`registry.ts`'s `TRANSFORM_PRIMITIVES`); Tier 2 = declarative YAML (`declarative/`), see next entry. |
| **`PrimitiveDefinition` (Tier 2 / declarative primitive)** | (v0.14.0) A YAML file (`.ai-context/primitives/*.yaml` or bundled) describing a transform via a constrained, `eval`-free template (`declarative/templateEngine.ts`) instead of TypeScript — loaded into the same `TRANSFORM_PRIMITIVES` registry as Tier 1, behind the identical `TransformPrimitive` interface. See §17.13. |
| **Primitive lifecycle status** | (v0.14.0) `draft → published → deprecated → retired` on a Tier-2 `PrimitiveDefinition`. Gates whether `selectTransformSpec()` offers it to the LLM (`isSelectable()`); does **not** gate whether it still compiles when an already-approved spec references it directly. See §17.14. |
| **Primitive version pinning / `primitiveVersion`** | (v0.14.0) An optional field on `TransformSpec` recording the exact `PrimitiveDefinition` revision a Pipeline Spec entity was authored against, resolved via a separate `VERSIONED_PRIMITIVES` history map (`registry.ts`) — so a later republish of that primitive can't silently change what an already-approved spec compiles to. |
| **Pipeline Spec** | (v0.14.0) The strict, `additionalProperties:false`-at-every-level YAML artifact (`.ai-context/problems/<id>/spec/pipeline.yaml`) synthesized from an approved BPS plus context — each entity's `transforms[]` is a list of `TransformSpec` primitive selections, deterministically compiled by `compilePipelineSpec()`. Distinct from the BPS: the BPS is the *problem*, the Pipeline Spec is the *deterministic solution shape*. See §5.7, §12 of `technical-design.md`. |
| **Design doc (Pipeline Spec)** | (v0.14.0) A deterministic, pure-function Markdown projection of a Pipeline Spec (`designDocGenerator.ts`'s `generateDesignDoc()`) — regenerable at any time from the spec, never hand-edited, never a second source of truth. |
| **Attachment extraction / `AttachmentExtract`** | (v0.14.0) LLM-based structured extraction over an uploaded attachment (`core/attachmentExtraction.ts`) feeding Pipeline Spec synthesis — degrades gracefully (never throws) if extraction fails, falling back to raw attachment text. |
| **`CredentialResolver` / `CredentialReference`** | (Designed 2026-09-17, not yet implemented — Phase 3a) A pluggable abstraction so a platform connection's credential can be sourced from an env var, VS Code SecretStorage, a CLI's own config, or an enterprise secrets manager, without `ConnectionManager`/the adapters knowing which. What's persisted is the `CredentialReference` (a pointer), never the resolved secret. See `technical-design.md` §4.8. |
| **`PrincipalType`** | (Designed 2026-09-17, not yet implemented — Phase 3a) `'interactive-user' \| 'service-account'` — a first-class dimension on credential resolution and auth-mode selection, so a headless/CI deployment and a developer at a workstation get distinct default paths through the same `CredentialResolver` abstraction. |
| **`PlatformCliDescriptor`** | (Designed 2026-09-17, not yet implemented — Phase 3a) The planned CLI-detection/enumeration contract for `snow`/`databricks`, mirroring `claudeCodeAdapter.ts`'s existing 3-tier cascade (explicit setting → PATH → none) and structured, non-throwing result shape. |
...
**New in v0.14.0, documented fresh in this pass:** Phases 1, 2, and 2B (i–iv) — the Deterministic Execution Core & Declarative Specification Framework (commits `4e52fd5`..`0c6a696`). Specifically: `DisposableRegistry`/real `deactivate()`; CI + ESLint; the Tier-1 `TransformPrimitive` registry (`rename_cast`/`dedup`/`incremental_load`) wired into 3 of 5 codegen agents ahead of the freehand-LLM/template fallback; Tier-2 declarative primitives (YAML + a constrained, `eval`-free template engine); the strict Pipeline Spec framework (`core/pipelineSpec/`) plus its deterministic Markdown design-doc generator, both derived from an approved BPS; attachment structured extraction (`AttachmentExtract`, closing the old raw-text-dump gap) and `SpecProvenance`'s previously-unused `'attachment'` source; the sidebar Primitives palette (list/preview/publish/deprecate) with `draft/published/deprecated/retired` lifecycle gating; the `primitiveManagementEnabled` opt-in governance gate; and `TransformSpec.primitiveVersion` pinning via a separate `VERSIONED_PRIMITIVES` history map. See §1, §3, §5.7, §12 of `technical-design.md`, and the new §17.13–17.15 recipes and §18 glossary terms added in this pass.

**Also designed in this pass, explicitly not yet implemented:** the Platform Onboarding & Credential Architecture (Phase 3a) — a `CredentialResolver`/`CredentialReference` abstraction spanning `interactive-user` and `service-account` principals, and a `PlatformCliDescriptor` detection cascade for `snow`/`databricks` mirroring `claudeCodeAdapter.ts`. Full detail in `requirements.md` §16.8/§17.6 and `technical-design.md` §4.8; tracked as R24–R26 below (§16) plus the pre-existing R1.

**Corrected in this pass:** §3's `src/spokes/`/`src/features/` section, which still described `src/features/**` as an existing (if legacy) directory — it was actually deleted in Phase 1 of this same v0.14.0 work. Renamed and rewritten to reflect current disk state.

**Not done in this pass:** none of this session's three documentation files (`requirements.md`, `technical-design.md`, this file) were committed to git as part of the writing itself — see the repository's commit history for when that landed, if at all, since a docs-only commit can lag the code it describes.
